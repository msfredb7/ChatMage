...
namespace CCC.Manager
{
    public class SoundManager : BaseManager<SoundManager>
    {
		...
		/// </summary>
		static public void PlayStaticSFX(AudioPlayable playable, float delay = 0, AudioSource source = null)
		{
			if (CheckResources_Instance())
				PlayNonMusic(playable, delay, source, instance.staticSFXSource);
		}
		...
		/// </summary>
		static public void PlayVoice(AudioPlayable playable, float delay = 0, AudioSource source = null)
		{
			if (CheckResources_Instance())
				PlayNonMusic(playable, delay, source, instance.voiceSource);
		}
		...
		/// Plays the audioplayable. Leave source to 'null' to play on the standard 2D SFX audiosource.
		/// </summary>
		static public void PlaySFX(AudioPlayable playable, float delay = 0, AudioSource source = null)
		{
			if (CheckResources_Instance())
				PlayNonMusic(playable, delay, source, instance.SFXSource);
		}
		...
		private static void PlayNonMusic(AudioPlayable playable, float delay, AudioSource source, AudioSource defaultSource)
		{
			if (playable == null)
				return;
			...
			if (delay > 0)
			{
				instance.StartCoroutine(PlayNonMusicIn(playable, delay, theSource));
				return;
			}
			else
			{
		...
		private static IEnumerator PlayNonMusicIn(AudioPlayable playable, float delay, AudioSource source)
		{
			yield return new WaitForSecondsRealtime(delay);
			playable.PlayOn (source);
		}
		...
		static private float Internal_PlayMusic(AudioPlayable playable, bool looping, float volumeMultiplier = 1)
		{
			...
			if (looping)
				playable.PlayLoopedOn (source, true);
			else
				playable.PlayOn (source);
		...
		static private void Internal_PlayMusicFaded(AudioPlayable playable, float fadeInDuration, bool looping = true, float volume = 1, float startingVolume = 0)
		{
			if (!CheckResources_Instance() || !CheckResources_MusicSource())
				return;
